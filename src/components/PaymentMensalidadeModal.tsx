...
function PaymentMensalidadeModalForm({
  ...
  const [fineStr, setFineStr] = useState(() =>
    m.waivesLateFees || !system.isLate ? '0.00' : round2(system.fine).toFixed(2),
  )
  const [interestStr, setInterestStr] = useState(() =>
    m.waivesLateFees || !system.isLate ? '0.00' : round2(system.interest).toFixed(2),
  )
  ...
  const total = useMemo(() => {
    if (m.waivesLateFees || !system.isLate) return m.liquidAmount
    return round2(m.baseAmount + fine + interest)
  }, [m.liquidAmount, m.baseAmount, m.waivesLateFees, system.isLate, fine, interest])

  const needsNote =
    !m.waivesLateFees &&
    system.isLate &&
    (round2(fine) !== round2(system.fine) || round2(interest) !== round2(system.interest))
  ...
  const handleConfirm = async () => {
    ...
    try {
      await onConfirm({
        ...
        adjustmentNotes: needsNote ? notes.trim() : undefined,
      })
      onClose()
    } catch {
        ...
        <div className="space-y-4 px-4 py-4 text-sm">
          ...
          <div className="grid grid-cols-2 gap-3 rounded-lg border border-slate-100 bg-slate-50/80 px-3 py-2 text-xs text-slate-700">
            ...
            {system.isLate && (
              <p className="col-span-2">
                Atraso: <span className="font-semibold">{system.late}</span> dia(s) — multa/juros
                calculados sobre o <strong>valor bruto</strong>.
              </p>
            )}
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="text-xs font-medium text-slate-700">
              ...
              <input
                ...
                min={0}
                step={0.01}
                disabled={!system.isLate || m.waivesLateFees}
                value={fineStr}
              ...
              />
              {system.isLate && !m.waivesLateFees && (
                <span className="mt-0.5 block text-[10px] text-slate-500">
                  Calculado: R$ {round2(system.fine).toFixed(2)}
                </span>
              )}
            </label>
            <label className="text-xs font-medium text-slate-700">
              ...
              {system.isLate && !m.waivesLateFees && (
                <span className="mt-0.5 block text-[10px] text-slate-500">
                  Calculado: R$ {round2(system.interest).toFixed(2)}
                </span>
              )}
            </label>
          </div>
          ...
          {needsNote && (
            <label className="block text-xs font-semibold text-amber-900">
              Observações da alteração (obrigatório)
              <textarea
                value={notes}
                onChange={(e) => setNotes(e.target.value)}
                rows={3}
                className="mt-1 w-full rounded-lg border border-amber-200 bg-amber-50/50 px-3 py-2 text-sm text-slate-900 outline-none ring-amber-500/30 focus:border-amber-500 focus:ring-2"
                placeholder="Ex.: Desconto de multa acordado com a direção…"
              />
            </label>
          )}
